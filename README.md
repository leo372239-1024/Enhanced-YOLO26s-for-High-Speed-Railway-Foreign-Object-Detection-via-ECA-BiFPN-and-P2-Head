...
- 项目简介：项目基于 YOLO26s 的改进版本，数据集用的是 RailFOD23 铁路异物检测数据集 。模型创新技术架构是通过引入 ECA 注意力机制、BiFPN 颈部网络 以及 P2 高分辨率检测头，提升YOLO26s模型对铁路场景中微小异物（如塑料袋、气球、鸟巢等）的检测性能，改进模型相较于Baseline，Precision提升3.47%。
